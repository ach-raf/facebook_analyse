...
subject = sys.argv[1]
if not os.path.exists('./result/%s/%s_input' % (subject, subject)):
    os.makedirs('./result/%s/%s_input' % (subject, subject))
    os.makedirs('./result/%s/%s_plots' % (subject, subject))
...
class GroupChat:
    ...
    def time_plot(self, bin_size=1, window=30):
        ...
        fig.savefig('./result/%s/%s_plots/all-user_lifetime_activity.png' % (subject, subject))
    ...
    def time_plot_user(self, names, bin_size=1, window=30):
        ...
        fig.savefig('./result/%s/%s_plots/lifetime_activity.png' % (subject, subject))
    ...
    def matrix_plot(self):
        ...
        fig.savefig('./result/%s/%s_plots/Conversation_Matrix_All_User.png' % (subject, subject))
    ...
    def word_plot(self, words, bin_size=30):
        ...
        fig.tight_layout()
        fig = self.fig_watermark(fig, 'Usage for %s/%s' % full_string)
        fig.savefig('./result/%s/%s_plots/Word_Usage_%s/%s.png' % (subject, full_string))
    ...
    def daily_plot(self, names=None, window=60):
        ...
        fig.savefig('./result/%s/%s_plots/%s.png' % (subject, subject, title))
    ...
    def weekly_plot(self, window=60):
        ...
        fig.savefig('./result/%s/%s_plots/Group_Daily_Weekly_Activity.png' % (subject, subject))
    ...
    def message_length_plot(self):
        ...
        fig.savefig('./result/%s/%s_plots/Message_Length.png' % (subject, subject))
    ...
    def word_length_plot(self):
        ...
        fig.savefig('./result/%s/%s_plots/All_User_Word_Length_Distribution.png' % (subject, subject))
    ...
    def messages_plot(self):
        ...
        img.save('./result/%s/%s_plots/Messages.png' % (subject, subject))
...
def messages_load():
    ...
    if not os.path.exists('./result/%s/%s_input/%s_messages.pkl' % (subject, subject, subject)) \
    ...
    # Load messages dicts
    with open('./result/%s/%s_input/%s_messages.pkl' % (subject, subject, subject), 'rb') as f:
